...
//	  buttonLightLED();
	  ...
	  // dac for all part except the last with the button
...
//	  HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_2, DAC_ALIGN_8B_R, SINOutput);
	  ...
	  /// delay for temperature change
	  // frequency goes down when temperature goes up
//	  for(int32_t i = 0;i<temperatureCelcius*100;i++);

	  // dekay for first part
//	  HAL_Delay(delay);


      GPIO_PinState buttonRead = HAL_GPIO_ReadPin(BLUE_BUTTON_GPIO_Port, BLUE_BUTTON_Pin);
      if(buttonCurrentState != buttonRead && buttonRead == GPIO_PIN_RESET){
      ...
      // last part change signal depending on button press
      HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_8B_R, currentSAWValue);
      if(ledCurrentState == GPIO_PIN_RESET){
    	  ...
    	  for(int32_t i = 0;i<temperatureCelcius*100;i++); // for loop use the cpu cycle to create the delays (NOP)
      }
